...
export default function PlayerGameTrendWidget() {
  ...
  const chartData = useMemo(() => {
    ...
    return rows.map((r) => {
      const oppRaw = (r.OPP_TEAM_ABBR ?? "").toString();
      const oppClean = oppRaw.toUpperCase().replace(/[^A-Z]/g, "");
      const opp = oppClean.length >= 3 ? oppClean.slice(-3) : oppClean;

      let label = opp
        ? `${opp}${r.IS_HOME ? " (H)" : r.IS_AWAY ? " (A)" : ""}`
        : "—";
      ...
      return {
        label,
        date: formatGameDate(r.GAME_DATE),
        value,
      };
    });
  ...
  const barW = lastN === 5 ? "70px" : "38px";
  const gap = lastN === 5 ? 8 : 4;
        ...
        <CardBody pt={3}>
          ...
          >
            {isLoadingLog ? (
              ...
              <Box>
                <Box position="relative" w="100%" px={2} py={2}>
                  {/* Bar area gutter */}
                  <Box pr="80px">
                    {/* Avg line */}
                    <Box
